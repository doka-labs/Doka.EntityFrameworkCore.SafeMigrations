...
public sealed partial class MySqlSafeMigrationIntegrationTests
{
    ...
    [Fact]
    public async Task ModelManagedDataBlocksUniquePrerequisiteEngineAndUnmodeledDependencyEdges()
    {
        var connectionString = await Fixture.CreateDatabaseAsync(CancellationToken.None);
        await ExecuteSqlAsync(
            connectionString,
            "CREATE TABLE `model_edge_roles` ("
            + "`id` int NOT NULL, `code` varchar(64) NOT NULL, `name` varchar(64) NULL, "
            + "PRIMARY KEY (`id`), UNIQUE KEY `ux_model_edge_roles_code` (`code`)) ENGINE=InnoDB;"
            + "CREATE TABLE `model_edge_audit` (`id` int NOT NULL, `role_id` int NOT NULL, "
            + "PRIMARY KEY (`id`), CONSTRAINT `fk_model_edge_audit_role` FOREIGN KEY (`role_id`) "
            + "REFERENCES `model_edge_roles` (`id`) ON DELETE CASCADE) ENGINE=InnoDB;"
            + "CREATE TABLE `model_edge_non_transactional` (`id` int NOT NULL, `name` varchar(64) NULL, "
        ...
        _ = missingPrerequisite.EnsureModelManagedDataFromModel(
            ...
            ["int"],
            new object?[,] { { 1 } });
        var nonTransactional = new MigrationBuilder(context.Database.ProviderName!);
        _ = nonTransactional.EnsureModelManagedDataFromModel(
            "model_edge_non_transactional",
            ["id"],
            ["int"],
            ["id", "name"],
            ["int", "varchar(64)"],
        ...
        var engineReport = await runner.AnalyzeAsync(
            context,
            nonTransactional.Operations,
            new SafeMigrationRunOptions("model-data-engine"),
            CancellationToken.None);
        ...
        _ = await Assert.ThrowsAsync<MySqlException>(() =>
            ExecuteOperationsAsync(context, unmodeledDependency.Operations, CancellationToken.None));
        ...
        Assert.Equal(SafeMigrationObservedState.Unsupported, Assert.Single(engineReport.Assessments).ObservedState);
    ...
    public async Task ModelManagedUpdateHandlesNullSourceAndMixedTargetRowsAtomically()
    {
        ...
        await ExecuteSqlAsync(
            connectionString,
            "CREATE TABLE `model_null_roles` (`id` int NOT NULL, `name` varchar(64) NULL, "
    ...
    public async Task OrderedChildDeleteDischargesOnlyTheExactLiveParentDependency()
    {
        ...
        await ExecuteSqlAsync(
            connectionString,
            "CREATE TABLE `model_roles` (`id` int NOT NULL, `name` varchar(64) NOT NULL, "
            + "PRIMARY KEY (`id`)) ENGINE=InnoDB;"
            + "CREATE TABLE `model_user_roles` (`id` int NOT NULL, `role_id` int NOT NULL, "
            + "PRIMARY KEY (`id`), CONSTRAINT `fk_model_user_roles_role` FOREIGN KEY (`role_id`) "
            + "REFERENCES `model_roles` (`id`) ON DELETE CASCADE) ENGINE=InnoDB;"
        ...
        await ExecuteOperationsAsync(context, exact.Operations, CancellationToken.None);
    ...
    public async Task ModelManagedDataTransitionsAreIdempotentAndRejectDrift()
    {
        ...
        await ExecuteSqlAsync(
            ...
            + "`id` int NOT NULL, `code` varchar(64) NOT NULL, `name` varchar(64) NULL, "
            + "PRIMARY KEY (`id`), UNIQUE KEY `ux_model_roles_code` (`code`)) ENGINE=InnoDB;"
        ...
        _ = await Assert.ThrowsAsync<MySqlException>(() =>
            ExecuteOperationsAsync(context, drift.Operations, CancellationToken.None));
